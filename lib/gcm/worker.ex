defmodule GCM.Worker do
  ...
  def handle_info(:connect_apple, %{config: config, ssl_opts: opts} = state) do
    ...
    case :ssl.connect(host, port, opts, timeout) do
      {:ok, socket} ->
        Logger.debug "[GCM] connected to #{address}"
        {:noreply, %{state | socket_apple: socket, counter: 0}}
      {:error, reason} ->
        Logger.error "[GCM] failed to connect #{address}, reason given: #{inspect reason}"
  ...
  def handle_info({:ssl_closed, socket}, %{socket_apple: socket} = state) do
    Logger.debug "[GCM] Apple socket was closed"
  ...
  def handle_info({:ssl_closed, socket}, %{socket_feedback: socket} = state) do
    ...
    Logger.debug "[GCM] Feedback socket was closed. Reconnect in #{state.config.feedback_interval} seconds."
  ...
  def handle_info({:ssl, socket, data}, %{socket_feedback: socket} = state) do
    case <<state.buffer_feedback :: binary, data :: binary>> do
      <<time :: 32, length :: 16, token :: size(length)-binary, rest :: binary>> ->
        %GCM.Feedback{time: time, token: Base.encode16(token)}
  ...
  def handle_call(%GCM.Message{token: token} = msg, _from, state) when byte_size(token) != 64 do
    GCM.Error.new(msg.id, 5)
    |> state.config.callback_module.error()
    {:reply, :ok, state}
  end
  def handle_call(%GCM.Message{} = msg, _from, %{config: config} = state) do
  ...
  defp get_config(pool_conf) do
    ...
    global_conf = Application.get_all_env :gcm
